...
from shield_ai.infrastructure.database.models import ProductModel, InventoryModel, BatchModel, SaleModel, ShrinkageCoefficientModel
...
class CalibrateCoefficientsUseCase:
    ...
    def _calibrate_product(self, product: ProductModel) -> Dict:
        ...
        if len(data) < 3:
            return {
                'a': 0.05, 'b': 0.1, 'c': 0.01,
                'rmse': None, 'data_points': len(data),
                'status': 'стандартные'
            }
        ...
        return {
            'a': a, 'b': b, 'c': c, 'rmse': rmse,
            'data_points': len(data), 'status': 'калиброван'
        }
    
    def _get_calibration_data(self, product_id: int) -> List[Dict]:
    ...
    def _calculate_portion(self, point: Dict, a: float, b: float, c: float) -> float:
    ...
    def _save_coefficients(self, product_id: int, coeffs: Dict) -> None:
