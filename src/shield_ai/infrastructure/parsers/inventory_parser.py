...
class InventoryParser:
    ...
    def __init__(self, groups_file: Optional[str] = None, logger: Optional[Logger] = None):
        """
        ...
        if groups_file:
            ...
            self.logger.info(f"Файл групп не указан, используется путь по умолчанию: {default_groups_file}")
    ...
    def _load_groups(self, file_path: str) -> Set[str]:
        ...
        try:
            ...
            self.logger.debug(f"Загружено {len(known_groups)} групп из справочника: {file_path}")
            return known_groups
        except FileNotFoundError:
            self.logger.error(f"Файл справочника групп не найден: {file_path}")
            return set()
        except PermissionError:
            self.logger.error(f"Нет прав доступа к файлу справочника групп: {file_path}")
            return set()
    ...
    def _classify_row(self, name_str: str) -> str:
       ...
       if 'ведомость по партиям' in lower_name_str or \
          'параметры:' in lower_name_str or \
          'отбор:' in lower_name_str:
           row_type = 'meta'
           self.logger.debug(f"Row '{name_str}' classified as: '{row_type}'")
           return 'meta'

       if name_str in ['Склад', 'Номенклатура', 'Документ движения', 'Партия.Дата прихода']:
           row_type = 'header'
           self.logger.debug(f"Row '{name_str}' classified as: '{row_type}'")
           return 'header'
    ...
    def _validate_balance(self, begin: float, in_qty: float, out_qty: float, end: float, tolerance: float = 0.001) -> Dict:
    ...
    def _handle_product_row(self, name_str: str, current_group: dict, stats: dict, begin: float, in_qty: float, out_qty: float, end: float, warnings: list = None) -> dict:
        ...
        product = {
            ...
            'batches': [],
            'qty_summary': {
                'begin': begin, 'in': in_qty, 'out': out_qty, 'end': end
            }
        }

        if not current_group:
            self.logger.warning(f"Product '{name_str}' skipped due to no current group.")
            warning_msg = f"Найден товар '{name_str}' без определенной группы. Товар будет проигнорирован."
            self.logger.warning(warning_msg)
            warnings.append(warning_msg)
            # Не добавляем продукт в группу и не обновляем статистику
    ...
    def _handle_batch_row(self, idx: int, name_str: str, current_group: dict, current_product: dict, stats: dict, begin: float, in_qty: float, out_qty: float, end: float, warnings: list = None) -> Optional[dict]:
        """Обрабатывает строку типа 'batch'."""
        if warnings is None:
            warnings = []
            
        if not current_product:
            self.logger.warning(f"Batch '{name_str}' skipped due to no current product.")
    ...
    def _handle_document_row(self, idx: int, name_str: str, current_product: Optional[dict], current_batch: Optional[dict], stats: dict, expense_operations: list, doc_in_qty: float, doc_out_qty: float, warnings: list = None):
        """Обрабатывает строку типа 'document'."""
        if warnings is None:
            warnings = []
            
        if not current_product:
            self.logger.warning(f"Document '{name_str}' skipped due to no current product.")
        ...
        if doc_type == 'expense':
            ...
            if doc_out_qty > 0:
                ...
                self.logger.debug(f"[ОТЛОЖЕННЫЙ РАСХОД] '{name_str}', кол-во: {doc_out_qty}")
    ...
    def _process_row(self, idx: int, row: list, context: dict, stats: dict, expense_operations: list, sections: list, warnings: list = None):
        ...
        current_group_name = context['current_group']['name'] if context['current_group'] else 'None'
        current_product_name = context['current_product']['name'] if context['current_product'] else 'None'
        self.logger.debug(f"Processing row {idx}: name='{name_str}', type='{row_type}', current_group='{current_group_name}', current_product='{current_product_name}'")
        
        if not context['found_header_row'] and row_type == 'header':
            context['header_indices'] = self._find_header_indices(row)
            if all(col in context['header_indices'] for col in ['begin', 'in', 'out', 'end']):
                context['found_header_row'] = True
                self.logger.debug(f"Найдены заголовки колонок в строке {idx + 1}: {context['header_indices']}")
            return context
        ...
        if row_type in ['product', 'batch'] and context['found_header_row']:
            ...
            begin = self._safe_to_float(row[self.INDEX_BEGIN]) if len(row) > self.INDEX_BEGIN else 0.0
            in_qty = self._safe_to_float(row[self.INDEX_IN]) if len(row) > self.INDEX_IN else 0.0
            out_qty = self._safe_to_float(row[self.INDEX_OUT]) if len(row) > self.INDEX_OUT else 0.0
            end = self._safe_to_float(row[self.INDEX_END]) if len(row) > self.INDEX_END else 0.0
        ...
        if row_type == 'warehouse':
            ...
            product_result = self._handle_product_row(name_str, context['current_group'], stats, begin, in_qty, out_qty, end, warnings)
            ...
            context['current_batch'] = self._handle_batch_row(idx, name_str, context['current_group'], context['current_product'], stats, begin, in_qty, out_qty, end, warnings)
        
        elif row_type == 'document':
            self._handle_document_row(idx, name_str, context['current_product'], context['current_batch'], stats, expense_operations, in_qty, out_qty, warnings)
    ...
    def parse_file(self, file_path: str) -> Dict:
        ...
        for idx, row_cells in enumerate(ws.iter_rows()):
            ...
            context = self._process_row(idx, row, context, stats, expense_operations, sections, warnings)
            ...
            for section in sections:
                for prod in section['products']:
                    if prod['name'] == product_name:
                        self._apply_fifo_expense(prod, quantity, document_name, warnings)
                        product_found = True
                        break
                if product_found:
                    break
    ...
    def export_to_markdown(self, data: Dict, output_file: str):
        ...
        for section in data.get('sections', []):
            md_lines.append(f"\n## {section['name']}\n")
            md_lines.append("| Товар | Нач. остаток | Приход | Расход | Кон. остаток | Партий |")
            md_lines.append("|---|---|---|---|")
            ...
            for product in section['products']:
                if product['batches']:
                    ...
                    for batch in product['batches']:
                        ...
                        b, i, o, e = qty_raw['begin'], qty_raw['in'], qty_raw['out'], qty_raw['end']
                        
                        print(f"  Партия {batch['batch_code']}:")
                        print(f"    Начало: {b:<10.4f} Приход: {i:<10.4f} Расход: {o:<10.4f} Конец: {e:<10.4f}")
                        
                        if not batch['validation']['valid']:
                            print(f"    ❌ ОШИБКА БАЛАНСА: {batch['validation']['error']}")
                        else:
                            print(f"    ✅ Баланс корректен (допуск {batch['validation']['diff']:.4f})")
