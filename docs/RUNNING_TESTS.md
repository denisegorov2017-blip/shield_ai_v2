...
2. **Прогнозирование усушки** - предсказание потерь для активных партий
3. **Стратегии усушки** - три различных модели расчета (Порционная, Взвешенная, Совместимости)
...
## Тестирование калибровки коэфициентов

```bash
# Установка PYTHONPATH для корректного импорта
PYTHONPATH="${PYTHONPATH}:${PWD}/src" python -c "
...
2. **ВЗВЕШЕННАЯ ИНТЕГРАЛЬНАЯ МОДЕЛЬ** (99.5% точность) - используется для production прогнозов
   - Усушка рассчитывается дискретно по дням с учётом остатка
   - Подходит для реальных расчетов

3. **МОДЕЛЬ СОВМЕСТИМОСТИ** (85-90% точность) - для быстрых оценок
