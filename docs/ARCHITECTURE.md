# 🏭 Shield AI v2.0 - Архитектура

## Clean Architecture принципы

Shield AI построен на **Clean Architecture** - полное разделение слоёв и зависимостей.

```python
src/shield_ai/
├── domain/              # 🎯 Бизнес-логика (независимая)
│   ├── entities/        # Доменные сущности
│   │   ├── product.py
│   │   ├── batch.py
│   │   └── shrinkage_profile.py
│   ├── repositories/    # Интерфейсы
│   ├── shrinkage/       # Модели усушки
│   │   └── strategies.py  # 3 стратегии
│   └── validation/      # Валидация
├── application/         # 📋 Use Cases
│   └── use_cases/
│       ├── audit_inventory.py
│       ├── calibrate_coefficients.py
│       └── forecast_shrinkage.py
├── infrastructure/      # 🔧 Внешние зависимости
│   ├── database/        # SQLAlchemy 2.0
│   │   ├── base.py
│   │   ├── models.py     # ORM модели
│   │   └── session.py
│   ├── parsers/         # Парсеры
│   │   └── inventory_parser.py
│   ├── exporters/       # Экспортеры
│   │   ├── json_exporter.py
│   │   ├── markdown_exporter.py
│   │   └── sqlite_exporter.py
│   └── config/          # DI контейнер
└── presentation/        # 🖥️ UI/API
    ├── cli/             # CLI интерфейс
    │   └── main.py
    └── ui/
        └── pages/       # Streamlit страницы
```

______________________________________________________________________

## Слои архитектуры

### 1. Domain Layer (Доменный слой)

**Назначение**: Чистая бизнес-логика без зависимостей

**Содержит**:

- `entities/` - Доменные сущности (Product, Batch, ShrinkageProfile)
- `shrinkage/` - Модели усушки (3 стратегии)
- `repositories/` - Интерфейсы репозиториев
- `validation/` - Валидация бизнес-правил

**Принципы**:

- Не зависит от фреймворков
- Не знает о БД, UI, API
- Только бизнес-логика

### 2. Application Layer (Слой приложения)

**Назначение**: Сценарии использования (Use Cases)

**Содержит**:

- `use_cases/` - Бизнес-сценарии
  - `AuditInventoryUseCase` - Аудит остатков
  - `CalibrateCoefficientsUseCase` - Калибровка
  - `ForecastShrinkageUseCase` - Прогнозирование
- `dto/` - Data Transfer Objects

**Принципы**:

- Зависит только от Domain
- Оркестрирует доменную логику

### 3. Infrastructure Layer (Инфраструктурный слой)

**Назначение**: Реализация внешних зависимостей

**Содержит**:

- `database/` - SQLAlchemy 2.0 ORM
- `parsers/` - Парсеры (например, InventoryParser)
- `exporters/` - Экспортеры (JsonExporter, MarkdownExporter, SQLiteExporter)
- `config/` - DI контейнер

**Принципы**:

- Реализует интерфейсы из Domain
- Работает с внешними системами

### 4. Presentation Layer (Слой представления)

**Назначение**: UI и API

**Содержит**:

- `cli/` - CLI интерфейс (main.py)
- `ui/pages/` - Streamlit страницы (1_parse.py, 2_calibrate.py, и т.д.)
- `api/` - FastAPI endpoints (опционально)

**Принципы**:

- Использует Use Cases
- Не содержит бизнес-логику

______________________________________________________________________

## Ключевые принципы

### 1. Dependency Inversion

Зависимости направлены **внутрь** (от внешних слоёв к внутренним):

```text
Presentation → Application → Domain ← Infrastructure
```

### 2. SOLID принципы

- **S**ingle Responsibility - одна ответственность
- **O**pen/Closed - открыто для расширения
- **L**iskov Substitution - подстановка наследников
- **I**nterface Segregation - разделение интерфейсов
- **D**ependency Inversion - инверсия зависимостей

### 3. Strategy Pattern

Три стратегии усушки реализованы через Strategy Pattern.

______________________________________________________________________

## Технологии

### SQLAlchemy 2.0 (синхронный)

- `Mapped` типизация
- `mapped_column` для колонок
- `relationship` для связей
- `future=True` для SQLAlchemy 2.0 стиля

### MyPy Strict Mode

- Строгая типизация
- `py.typed` маркер
- Type hints везде

______________________________________________________________________

**🛡️ Shield AI v2.0** - production-ready архитектура
